...
void VTrack::calculate_input_transfer(int asset_w, int asset_h,
...
//printf("VTrack::calculate_input_transfer %jd\n", position);

// get camera center in asset
	automation->get_camera(&auto_x, &auto_y, &auto_z,
		position, direction);
...
void VTrack::calculate_output_transfer(int64_t position, int direction,
	float &in_x, float &in_y, float &in_w, float &in_h,
	float &out_x, float &out_y, float &out_w, float &out_h)
{
...
int VTrack::get_projection(float &in_x1, float &in_y1, float &in_x2, float &in_y2,
	...
	int frame_w, int frame_h, int64_t real_position, int direction)
{
	float center_x, center_y, center_z;
	float x[4], y[4];
	...
	in_x1 = x[0];  in_y1 = y[0];  in_x2 = x[1];  in_y2 = y[1];
	out_x1 = x[2]; out_y1 = y[2]; out_x2 = x[3]; out_y2 = y[3];
	return 0;
}
