	...
	int vertical_justify)
{
	Window tempwin;
	int new_x, new_y;
	int top_x0 = top_level->get_screen_x(0, -1);
	int top_y0 = top_level->get_screen_y(0, -1);
	int top_x1 = top_x0 + top_level->get_screen_w(0, -1);
	int top_y1 = top_y0 + top_level->get_screen_h(0, -1);
